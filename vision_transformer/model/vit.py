...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_weights = torch.softmax(scores / (cfg['d_head'] ** 0.5), dim=-1)  # [b,h,p,p]
...
class VisionTransformer(nn.Module):
    def __init__(self, patch_size=16, img_size=224, in_channels=3, num_classes=1000):
        super().__init__()
        self.patch_size = patch_size
        self.num_patches = (img_size // patch_size) ** 2

        self.embedding = Embedding(patch_size, img_size, in_channels)
        self.encoder_layers = nn.ModuleList([Encoder() for _ in range(cfg['num_encoder_layer'])])
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        x = x.mean(dim=1)  # Global average pooling [b,num_patches,d_model] -> [b,d_model]
